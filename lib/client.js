...
module.exports = {
  ...
  async apply(ctx) {
    ...
    function GuardianPage() {
      var st = React.useState({ tab: 'installed', residues: [], plugins: [], health: null, loading: false, notice: null, confirm: null, loaded: {}, snapshots: [] });
      ...
      function loadSnapshots() {
        rpc('listSnapshots').then(function(r){
          upd({ snapshots: r });
        }).catch(function(){ /* 静默失败 */ });
      }

      function doRestore(snapPath, pluginName) {
        upd({ confirm: {
          title: '恢复到卸载前',
          body: '即将把配置文件恢复到卸载「' + pluginName + '」之前的状态。这会覆盖当前的配置文件。恢复后需要手动重启 DSH 服务。',
          confirmLabel: '确认恢复',
          danger: true,
          onConfirm: function() {
            upd({ confirm: null, loading: true });
            setNotice('正在恢复配置...');
            rpc('restoreSnapshot', { snapshotPath: snapPath }).then(function(r){
              if (r.success) {
                setNotice('恢复成功。请手动重启 DSH 服务使配置生效。', 'ok');
              } else {
                upd({ loading: false });
                setNotice('恢复失败：' + (r.message || '未知错误'), 'err');
              }
            }).catch(function(e){
              upd({ loading: false });
              setNotice('恢复失败：' + (e.message || '未知错误'), 'err');
            });
          },
          onCancel: function() { upd({ confirm: null }); }
        }});
      ...
      React.useEffect(function(){
        if (!state.loaded.installed) loadPlugins();
        loadSnapshots();
      }, []);
      ...
      kids.push(h('p', { className: 'pg-intro' }, '点击「卸载」即可安全移除插件，系统会自动备份配置并清理所有残留文件。'));
      ...
      if (!s.loading) {
        if (s.plugins.length === 0) {
          ...
          kids.push(h('ul', { className: 'pg-cards' }, cards));
        }
      }

      // 快照恢复区域：如果有历史快照，显示"如果卸载后出了问题可以恢复"
      if (!s.loading && s.snapshots.length > 0) {
        kids.push(h('div', { className: 'pg-hint', style: { marginTop: '16px', borderTop: '1px solid var(--dsw-alias-border-l1)', paddingTop: '12px' } },
          '如果卸载后重启遇到问题，可以点击下方「恢复」把配置还原到卸载前：'
        ));
        var snapCards = s.snapshots.map(function(snap){
          return h('li', { key: snap.snapshotPath, className: 'pg-card' },
            h('div', { className: 'pg-cardRow' },
              h('div', null,
                h('div', { className: 'pg-cardName' }, snap.pluginName),
                h('div', { className: 'pg-cardMeta' }, '卸载时间 ' + snap.timeLabel)
              ),
              h('div', { className: 'pg-cardTrailing' },
                h('button', {
                  className: 'pg-btn pg-btnOutline',
                  onClick: function(){ doRestore(snap.snapshotPath, snap.pluginName); },
                  disabled: s.loading
                }, icon('IconRefreshOutline14', 14), ' 恢复')
              )
            )
          );
        });
        kids.push(h('ul', { className: 'pg-cards' }, snapCards));
      }
