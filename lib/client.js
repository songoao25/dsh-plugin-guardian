  ...
  async apply(ctx) {
    ...
    function GuardianPage() {
      ...
      React.useEffect(function(){
        ...
        if (state.tab === 'health' && !state.loaded.health) { runHealth(); upd({ loaded: Object.assign({}, state.loaded, { health: true }) }); }
      }, [state.tab]);
      ...
      kids.push(h('p', { className: 'pg-intro' }, '安全地卸载插件、自动清理残留、重启前体检。全程图形操作，无需命令行。'));

      var tabs = [
        { id: 'installed', label: '已安装插件' },
        { id: 'residue', label: '残留清理' },
        { id: 'health', label: '启动体检' },
      ];
      ...
      if (s.tab === 'health' && !s.loading && s.health) {
        var hp = s.health.port3080 === 'free';
        var hc = s.health.cordisConfig === 'ok';
        var hb = s.health.bundles === 'ok';
        var allOk = hp && hc && hb;

        if (allOk) {
          kids.push(h('div', { className: 'pg-notice pg-noticeOk' },
            icon('IconCheckOutline16', 14), ' 所有检查项均通过，可以放心启动。'
          ));
        }

        var hrows = [];
        hrows.push(h('div', { key: 'port', className: 'pg-healthRow' },
          h('span', { className: 'pg-healthLabel' }, icon('IconDataOutline16', 14), ' 端口 3080'),
          h('span', { className: 'pg-healthValue' },
            h('span', { className: 'pg-dot ' + (hp ? 'pg-dotOk' : 'pg-dotErr') }),
            hp ? '空闲' : '被占用'
          )
        ));
        hrows.push(h('div', { key: 'config', className: 'pg-healthRow' },
          h('span', { className: 'pg-healthLabel' }, icon('IconCodeOutline16', 14), ' 组合配置'),
          h('span', { className: 'pg-healthValue' },
            h('span', { className: 'pg-dot ' + (hc ? 'pg-dotOk' : 'pg-dotErr') }),
            hc ? '正常' : '缺失'
          )
        ));
        hrows.push(h('div', { key: 'bundle', className: 'pg-healthRow' },
          h('span', { className: 'pg-healthLabel' }, icon('IconCordisPluginOutline14', 14), ' Bundle 状态'),
          h('span', { className: 'pg-healthValue' },
            h('span', { className: 'pg-dot ' + (hb ? 'pg-dotOk' : 'pg-dotWarn') }),
            hb ? '正常' : '需检查'
          )
        ));
        kids.push(h('div', { className: 'pg-card', role: 'tabpanel' }, hrows));

        if (!hp) kids.push(h('div', { className: 'pg-hint' }, '端口被占用通常是因为旧进程未完全退出。请先停止旧进程再启动，或等待几秒后重试。'));
        if (!hc) kids.push(h('div', { className: 'pg-hint' }, '组合配置文件缺失可能导致启动失败。请检查 ~/.dsh/profiles/web/cordis.yml 是否存在。'));
        if (!hb) kids.push(h('div', { className: 'pg-hint' }, 'Bundle 状态异常表示有插件包缺失或损坏。建议运行 pnpm install 重新安装依赖，或卸载问题插件。'));
      }
