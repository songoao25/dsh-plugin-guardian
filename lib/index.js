...
// 自动清理旧快照：保留最近 maxKeep 个，更老的删除。
function cleanOldSnapshots(maxKeep) {
  const snapBase = join(dshPath, '.plugin-guard-snapshots')
  let entries = []
  try { entries = readdirSync(snapBase) } catch (e) { return }
  // 按目录名排序（目录名以时间戳开头，字典序=时间序）
  entries.sort()
  while (entries.length > maxKeep) {
    const old = entries.shift()
    try { rmSync(join(snapBase, old), { recursive: true, force: true }) } catch (e) { /* 忽略 */ }
  }
}

// 列出所有快照（供前端显示"上次卸载了什么"）。
function listSnapshots() {
  const snapBase = join(dshPath, '.plugin-guard-snapshots')
  const result = []
  let entries = []
  try { entries = readdirSync(snapBase) } catch (e) { return result }
  entries.sort().reverse() // 最新的在前
  for (const name of entries) {
    const dir = join(snapBase, name)
    // 解析目录名：<timestamp>-<pluginName>
    const dashIdx = name.indexOf('-')
    const ts = dashIdx > 0 ? parseInt(name.slice(0, dashIdx), 10) : 0
    const pluginName = dashIdx > 0 ? name.slice(dashIdx + 1) : name
    result.push({
      snapshotPath: dir,
      pluginName: pluginName,
      timestamp: ts,
      timeLabel: ts > 0 ? new Date(ts).toLocaleString('zh-CN') : name,
    })
  }
  return result
}

// 恢复快照：把快照中的配置文件拷回 profile 目录。
function restoreSnapshot(args) {
  const snapPath = (args && args.snapshotPath) || ''
  if (!snapPath || !snapPath.startsWith(join(dshPath, '.plugin-guard-snapshots'))) {
  ...
  const restored = []
  for (const f of SNAPSHOT_FILES) {
    const src = join(snapPath, f)
    const dst = join(profilePath, f)
    if (existsSync(src)) {
      try { copyFileSync(src, dst); restored.push(f) } catch (e) { /* 忽略 */ }
    }
  }
  return {
    success: restored.length > 0,
    message: restored.length > 0 ? 'restored ' + restored.length + ' files' : 'no files to restore',
    restored,
  }
}
...
const ROUTES = { listPlugins, scanResidues, cleanFiles, healthCheck, createSnapshot, uninstallPlugin, listSnapshots, restoreSnapshot }
const MUTATING = { cleanFiles: true, uninstallPlugin: true, restoreSnapshot: true } // 修改状态的方法需同源
